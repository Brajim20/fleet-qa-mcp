...
For newly found bugs, call `build_issue_url` with the evidence to get a prefilled report URL:
...
- If `investigate` returns an error, check the issue number is correct and the
  instance is reachable (`fleet_request path=/api/v1/fleet/version`).
